...
** Last update Thu Mar 16 09:40:31 2017 Enguerrand Arminjon
*/
...
char		**load_shape_on_top(t_fall *f, char **map,
				    t_tetris *tetris, int j)
{
  ...
  i = -1;
  a = 1;
  center = ((tetris->t_debug->col + 2) / 2) - (f->weight - f->weight % 2) / 2;
  while (f->shape[++i] != NULL)
    {
      b = center;
      j = -1;
      while (f->shape[i][++j] != '\0')
