...
** Last update Mon Mar 13 09:14:08 2017 Enguerrand Arminjon
*/
...
int		l_num(char **av, int *i, t_tetris *tetris)
{
  ...
  if ((my_strcmp(av[*i], "-l") == 0) && av[*i + 1] != '\0'
    ...
    {
      tetris->t_debug->num = av[*i + 1];
      if ((my_getnbr(tetris->t_debug->num) < 0))
	return (84);
      *i = *i + 1;
      a = 1;
    }
    ...
    {
      tetris->t_debug->num = find_cmds(av[*i], '=');
      a = 1;
    }
  else
    return (84);
...
int		map_size(char **av, int *i, t_tetris *tetris)
{
  ...
  if (((my_strcmp_2(av[*i], "--map-size", '=')) == 0) && a == 0)
    {
      tetris->t_debug->row = my_getnbr(find_nb(av[*i], '='));
      tetris->t_debug->col = my_getnbr(find_nb(av[*i], ','));
      a = 1;
    }
  else
    return (84);
