...
** Last update Tue Mar 14 13:45:40 2017 Enguerrand Arminjon
*/
...
void	display_color_map(char **map, t_tetris *t, int i, int j)
{
  while (map[i][++j] != '\0')
      ...
      if (map[i][j] == '_' || map[i][j] == '.')
	{
	  ...
	  mvprintw(t->rn - 1, (t->cn - t->t_debug->col) / 2 + j, "%c", ' ');
	}
...
void	print_game_info_tetris(t_tetris *t, t_debug *d)
{
  int	i;

  i = t->max_high;
  mvwprintw(stdscr, i + 1, 1, "%s", "------------------------");
  mvwprintw(stdscr, i + 3, 1, "%s", "|                      |");
  mvwprintw(stdscr, i + 4, 1, "| High Score : %i\t|", t->high_score);
  mvwprintw(stdscr, i + 5, 1, "| Score      : %i\t|", t->score);
  mvwprintw(stdscr, i + 6, 1, "%s", "|                      |");
  mvwprintw(stdscr, i + 7, 1, "| Lines      : %i\t|", t->line);
  mvwprintw(stdscr, i + 8, 1, "| Level      : %i\t|", d->level + t->line / 10);
  mvwprintw(stdscr, i + 9, 1, "%s", "------------------------");
}
...
void	small_window_near(t_tetris *t)
{
  ...
  print_game_info_tetris(t, t->t_debug);
}
...
void	display_game(char **tetri_map, t_tetris *t, int i, int nw)
{
  ...
  load_color();
  if (row < t->t_debug->row + 2 || col < t->t_debug->col)
    enlarge_map(row, col, t);
  row = (row / 2) - (t->t_debug->row / 2);
  t->cn = col;
  clear();
  while (tetri_map != NULL && i < t->t_debug->row + 2)
    {
      if (t->t_debug->debug[0] != 'f')
	mvprintw(row - 1, (col - t->t_debug->col) / 2, "%s", tetri_map[i]);
      else
	{
	  t->rn = row;
