...
** Last update Thu Mar 16 09:38:36 2017 Enguerrand Arminjon
*/
...
void		speed_of_the_game(t_tetris *tetris)
{
  while (g_my_time >= 10)
    {
      if (tetris->time->my_time - 40 > 0)
	tetris->time->my_time = tetris->time->my_time - 40;
      else
	tetris->time->my_time = tetris->time->my_time - 1;
      g_my_time = g_my_time - 10;
    }
}

char		**fall_one_line(char **tetri_map, t_tetris *tetris)
{
				  ...
				  t_tetris *tetris, int ret)
{
  ...
  fall.shape = NULL;
  tetris->time->my_time = 1000 - tetris->t_debug->level * 40;
  ...
  while (check_game(tetri_map) == 0)
	...
	tetri_map = convert_map(tetri_map);
      tetri_map = my_delete_line(tetri_map, tetris->t_debug->row, tetris, 0);
    }
  return (SUCCESS);
}
...
int	display_ncurses(t_shape *shape, t_tetris *tetris)
{
  ...
  tetris->f_next = 0;
  g_my_time = 0;
  tetris->t_debug->level = my_getnbr(tetris->t_debug->num);
  tetris->line = 0;
  tetri_map = NULL;
  if ((initscr()) == NULL
  ...
  keypad(stdscr, TRUE);
  nodelay(stdscr, TRUE);
  tetri_map = init_map_game(tetri_map, tetris->t_debug, 0);
