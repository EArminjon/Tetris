...
** Last update Sun Mar 19 09:55:09 2017 Enguerrand Arminjon
*/
...
{
  if (d_shape->high > tetris->row || d_shape->weight > tetris->col)
    return (84);
  return (nb);
}
				   ...
				   t_tetris *tetris)
{
  ...
  if ((fd = open(f, O_RDONLY)) == -1)
    return (OPEN_FAIL);
  d_shape.my_tab = my_put_in_tab(&d_shape, fd);
...
char		**display_tetriminos(t_tetris *tetris, int i, int deb)
{
  DIR		*rep;
  struct dirent	*entry;
  char		**my_tab;
