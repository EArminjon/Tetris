...
** Last update Sat Mar 18 16:12:49 2017 Enguerrand Arminjon
*/
...
char	**game_cmds(char **tetri_map, t_tetris *tetris, t_fall *fall, char *s)
{
  ...
  if (s == NULL)
    tetris->keys = 0;
  else if ((my_strstr(s, tetris->quit)) == 0)
    {
      tetris->keys = -2;
      return (NULL);
    }
  else if ((my_strstr(s, tetris->left)) == 0)
    tetri_map = move_my_shape_left(tetri_map, tetris, fall);
  else if ((my_strstr(s, tetris->right)) == 0)
    tetri_map = move_my_shape_right(tetri_map, tetris, fall);
  else if ((my_strstr(s, tetris->top)) == 0)
    tetri_map = my_rotate_shape(tetri_map, tetris, fall);
  else if ((my_strstr(s, tetris->drop)) == 0)
    tetri_map = my_drop_shape(tetri_map, tetris);
  else if ((my_strstr(s, tetris->pause)) == 0)
